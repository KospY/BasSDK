...
namespace Needle
{
    ...
    public class MarkdownShaderGUI : ShaderGUI
    {
        ...
        private VariantCollectionWrapper variantCollectionWrapper;
#endif
        ...
        public override void OnClosed(Material material)
        {
            ...
            //Debug.Log("OnClosed: " + material.name);
            //foreach (var headerGroup in headerGroups)
            //{
            //    CloseGroup(headerGroup);
            //}
            ...
            if (headerGroups.Count < 1)
            {
                ...
                foreach (var prop in properties)
                {
                    ...
                    if (display.StartsWith(foldoutHeaderFormatStart) || display.Equals(foldoutHeaderFormat, StringComparison.Ordinal))
                    {
                        ...
                        if (prop.flags.HasFlag(MaterialProperty.PropFlags.HideInInspector)) continue; // Shadowood: Skips Header items if it has [HideInInspector] attribute
                            ...
                            switch (markdownType)
                            {
                                ...
                                case MarkdownProperty.Drawer:
                                    ...
                                    if (drawer != null)
                                    {
                                        ...
                                        if (referencedProps != null && drawer.ReplaceOriginal()) referencedProperties.AddRange(referencedProps);
                                    }
    ...
    */        
            ...
            void Platform()
            {
                ...
                //if (path.EndsWith(AssetSorceryShader.ASSETSORCERY_FILE_EXTENSION))
                {
                    ...
                    {
                        if (GUILayout.Button("Set Platform: " + item.ToString()))
                        {
...
#if ProjectCore
                variantCollection = EditorGUILayout.ObjectField("Collection", variantCollection, typeof(ShaderVariantCollection)) as ShaderVariantCollection;

                //if (EditorGUI.EndChangeCheck() && variantCollectionPC)
                {
                    //EditorPrefs.SetString("MarkdownShaderVC", AssetDatabase.GetAssetPath(variantCollection));
                }
                
                //Debug.Log("MarkdownShaderGUI: CollectLocalAndGlobalKeywords");
                /*
                var stripConfig = ShaderStripConfig.Get();

                if (stripConfig.TryGetPlatformShaderVariantCollection(out var collection))
                {
                    variantCollection = collection;
                    variantCollectionWrapper = new VariantCollectionWrapper(collection);
                }*/

                if (variantCollectionWrapper != null)
                {
                    var matKeywords = targetMat.enabledKeywords.ToList();
                    List<string> matKeys = new List<string>();
                    foreach (var localKeyword in matKeywords)
                    {
                        if (localKeyword.isValid) matKeys.Add(localKeyword.name);
                    }

                    var col = GUI.color;
                    var valid = KeywordsMatch(matKeys);

                    if (!valid)
                    {
                        GUI.color = Color.red;
                        EditorGUILayout.LabelField("Variants Invalid", EditorStyles.boldLabel);
                        foreach (var matKeyword in matKeywords)
                        {
                            EditorGUILayout.LabelField("Mat: " + matKeyword, EditorStyles.boldLabel);
                        }
                    }
                    else
                    {
                        EditorGUILayout.LabelField("Variants Valid", EditorStyles.boldLabel);
                    }
...
#if ProjectCore
            bool KeywordsMatch(List<string> matKeywordsFiltered)
            {
                bool matchedAll = false;
                bool shaderFound = false;
                
                foreach (var variant in variantCollectionWrapper)
                {
                    if (variant.Shader != targetMat.shader) continue;
                    shaderFound = true;

                    //EditorGUILayout.LabelField("variant: " + variant.PassType +":"+variant.Keywords, EditorStyles.boldLabel);
                    //Debug.Log("variant: "+ variant.PassType + " : " + variant.Keywords);
                    matchedAll = true;

                    var colKeywords = variant.KeywordsArray;

                    foreach (var keywordA in matKeywordsFiltered)
                    {
                        if (!colKeywords.Contains(keywordA))
                        {
                            //if(!missing.Contains(keywordA)) missing.Add(keywordA);
                            matchedAll = false;
                            break;
                        }
                    }
                    ...
                    //Debug.Log($"MarkdownVariants: {variant.Shader.name} {variant.PassType} {variant.Keywords}");
                }
                
                if(!shaderFound)EditorGUILayout.LabelField("Shader not present in collection", EditorStyles.boldLabel);
            ...
            void DrawDebugGroupContent()
            {
                ...
                if (variantCollection)
                {
                    ...
                    {
                        keywords.Add(localKeyword.Value.name);
                    }
                }
                ...
                {
                    GUI.color = Color.red;
                    EditorGUILayout.TextField(o, EditorStyles.miniLabel);
                }
                ...
                for (int i = 0; i < targetMat.passCount; i++)
                {
                    EditorGUILayout.TextField("pass: " + i + ":" + targetMat.GetPassName(i), EditorStyles.miniLabel);
                }*/
                ...
                {
                    /*
                    if (variantCollection.Contains(new ShaderVariantCollection.ShaderVariant(targetMat.shader, PassType.ScriptableRenderPipeline, keywords.ToArray())))
                    {
                        EditorGUILayout.TextField("Valid: "+ keywords.ToString(), EditorStyles.miniLabel);
                    }
                    else
                    {
                        EditorGUILayout.TextField("Invalid: "+ keywords.ToString(), EditorStyles.miniLabel);
                    }*/
                }
                catch (Exception e)
                {
                        ...
                        case MarkdownProperty.Drawer:
                            ...
                            if (parts.Length > 1)
                            {
                                ...
                                if (drawer)
                                {
                                    ...
                                    {
                                        Debug.LogError("Error in Custom Drawer \"" + objectName + "\":\n" + e.Message +" stack:"+e.StackTrace);
...
/* // Doesnt work?
        private void CloseGroup(HeaderGroup groupIn)
        {
            for (int i = 0; i < groupIn.properties.Count; i++)
            {
                var prop = groupIn.properties[i];
                var display = prop.displayName;
                display = display.TrimStart('-');
                switch (GetMarkdownType(display))
                {
        ...
        private void CollectLocalAndGlobalKeywords(Material material)
        {
            ...
            if (variantCollection == null)
            {
                ...
                /*
                var path = EditorPrefs.GetString("MarkdownShaderVC");
                if (!string.IsNullOrEmpty(path))
                {
                    variantCollection = AssetDatabase.LoadAssetAtPath<ShaderVariantCollection>(path);
                    //variantCollectionJSON = JsonUtility.FromJson<(File.ReadAllText(path));
                }*/
            }
#endif
        ...
        public void ShadowoodMaterialValidation(Material material)
        {
        ...
        protected virtual void MaterialChanged(MaterialEditor materialEditor, MaterialProperty[] properties)
        {
            ...
            foreach (var mat in materialEditor.targets)
            {
                ...
                //Debug.Log("MarkdownShaderGUI: MaterialChanged: " + material.name, material);
            }
        ...
        public void ValidateMaterial(Material material)
        {
            ...
            ShadowoodMaterialValidation(material);
            //Debug.Log("MarkdownShaderGUI: ValidateMaterial: " + material.name, material); // Shadowood
        }
