...
namespace Needle.ShaderGraphMarkdown
{
    public class GradientGeneratorDrawer : MarkdownMaterialPropertyDrawer
    {
        ...
        public override void OnDrawerGUI(MaterialEditor materialEditor, MaterialProperty[] properties, DrawerParameters parameters)
        {
            ...
            var newGradient = EditorGUILayout.GradientField(new GUIContent(parameters.ShowPropertyNames ? targetPropertyName : displayName, parameters.Tooltip), existingGradient, true);
            //var gradRect = GUILayoutUtility.GetLastRect();
            
            if (EditorGUI.EndChangeCheck()) AddToCache(newGradient);
            ...
            if (GUI.Button(buttonRect, targetProperty?.textureValue ? "Apply" : "Create")) ApplyRampTexture(targetMat, parameters.Get(0, targetPropertyName));
        ...
        {
            var textureCurrent = targetMaterial.GetTexture(propertyName);
            
            Texture2D gradientTexture = new Texture2D(GradientTexturePixelWidth, GradientTexturePixelHeight, TextureFormat.RGBAFloat, false, true)
            {
            ...
            gradientTexture = SaveAndGetTexture(targetMaterial, gradientTexture, textureCurrent);
        ...
        private static Texture2D SaveAndGetTexture(Material targetMaterial, Texture2D sourceTexture, Texture textureCurrent)
        {
            ...
            if(textureCurrent!=null) path = AssetDatabase.GetAssetPath(textureCurrent);

            if (path == "")
            {
                var suffix = hdr ? ".exr" : ".png";
                path = targetFolder + targetMaterial.name + sourceTexture.name + suffix;
            }
