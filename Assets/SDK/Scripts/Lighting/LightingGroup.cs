...
namespace ThunderRoad
{
    ...
    {
#if UNITY_EDITOR
        [UnityEditor.MenuItem("GameObject/BakedLODGroup/Update Lightmap Scales")]
        public static void UpdateLightMapScales()
        {
        ...
        {
            if (!enabled) return; // Shadowood: Validate will run scripts even if they are disabled; which is rude, could also check for isActiveAndEnabled
...
#if UNITY_EDITOR
                    UnityEditor.EditorApplication.delayCall += ApplyAll;
#endif
                }
            }
        }
        ...
        {
            ApplySkySceneSettings(lightingPreset);
        }
...
#if UNITY_EDITOR
            if(RenderSettings.sun) RenderSettings.sun.shadowAngle = lightingPreset.bakedShadowAngle;
            copyLevelCurrentPreset.bakedShadowAngle = lightingPreset.bakedShadowAngle;

            Lightmapping.lightingSettings.sampling = lightingPreset.multipleImportanceSamplingTool;
            copyLevelCurrentPreset.multipleImportanceSamplingTool = lightingPreset.multipleImportanceSamplingTool;

            Lightmapping.lightingSettings.directSampleCount = lightingPreset.directSamples;
            copyLevelCurrentPreset.directSamples = lightingPreset.directSamples;

            Lightmapping.lightingSettings.indirectSampleCount = lightingPreset.indirectSamples;
            copyLevelCurrentPreset.indirectSamples = lightingPreset.indirectSamples;

            Lightmapping.lightingSettings.environmentSampleCount = lightingPreset.environmentSamples;
            copyLevelCurrentPreset.environmentSamples = lightingPreset.environmentSamples;

            // Always Update max bounce before min bounce because there is a check that min < max
            Lightmapping.lightingSettings.maxBounces = lightingPreset.maxBounces;
            copyLevelCurrentPreset.maxBounces = lightingPreset.maxBounces;

            Lightmapping.lightingSettings.minBounces = lightingPreset.minBounces;
            copyLevelCurrentPreset.minBounces = lightingPreset.minBounces;
        ...
        public void ApplySkySceneSettings(LightingPreset lightingPreset)
        {
            if (lightingPreset == null)
            {
                Debug.LogWarningFormat(this, "Cannot apply sky scene settings, no lightingPreset is referenced");
                return;
            }
            ...
            if (lightingPreset.skybox == LightingPreset.State.Enabled)
            {
                ...
                if (lightingPreset.skyBoxMaterialProp.Mat) RenderSettings.skybox = lightingPreset.skyBoxMaterialProp.GetInstancedMat(0);
...
#endif
            // Blend RenderSettings
            RenderSettings.ambientIntensity = Mathf.Lerp(currentLightingPreset.ambientIntensity, lightingPreset.ambientIntensity, t);
            RenderSettings.subtractiveShadowColor = Color.Lerp(currentLightingPreset.shadowColor, lightingPreset.shadowColor, t);
            ...
            if (lightingPreset.fog != LightingPreset.State.NoChange)
            {
                ...
                else
                {
                    ...
                    if (lightingPreset.fog == LightingPreset.State.Enabled)
                    {
                        ...
                        //FogDistanceSmoothStep(fogStartDistance, fogEndDistance, lightingPreset.fogStartDistance, lightingPreset.fogEndDistance, t, out fogStartDistance, out fogEndDistance);  
                    ...
                    else
                    {
                        ...
                        //FogDistanceSmoothStep(fogStartDistance, fogEndDistance, Camera.main.farClipPlane, Camera.main.farClipPlane, t, out fogStartDistance, out fogEndDistance);
        ...
        {
            float diffFrom = endDistanceFrom - startDistanceFrom;
            float diffTo = endDistanceTo - startDistanceTo;

            float diff = Mathf.Lerp(diffFrom, diffTo, t);
        ...
        public void ClearLightingPreset()
        {
            ...
            lightingPreset.OnReleasePreset();
            lightingPreset = null;
        ...
        public void UpdateReferencesAndSaveAll(bool saveBakedReflectionProbe = true)
        {
            ...
            if (lightingPreset == null)
            {
                lightingPreset = LightingPreset.Create(this);
            }
            else
            {
                lightingPreset.GetSceneSettings();
            }
        ...
        public void SaveLightProbeVolumesOnly()
        {
            SaveLightProbeVolumesOnly(true);
        }
        ...
        public List<Issue> GetIssues(bool includeLongCheck, bool experimental)
        {
            ...
            if (lightingPreset == null)
            {
                Area tempArea = GetComponent<Area>();
                if (tempArea == null)
                {
                    tempArea = GetComponentInParent<Area>();
                }

                if (tempArea != null)
                {
                    AreaData areaData = tempArea.areaData.Data;
                    if (areaData != null)
                    {
                        lightingPreset = Catalog.EditorLoad<LightingPreset>(areaData.lightingPresetAddress);
                    }
                }
            }
