...
namespace UnityEditor.AI
{
    public class NavMeshAssetManager : ScriptableSingleton<NavMeshAssetManager>
    {
        ...
        static void CreateNavMeshAsset(NavMeshSurface surface)
        {
            ...
            //check if one already exists at the path
            if(File.Exists(combinedAssetPath))
            {
                //delete it
                File.Delete(combinedAssetPath);
            }
            combinedAssetPath = AssetDatabase.GenerateUniqueAssetPath(combinedAssetPath);
            Debug.Log($"Creating NavMesh asset at {combinedAssetPath}");
        ...
        void ClearSurface(NavMeshSurface navSurface)
        {
            ...
            if (assetToDelete)
            {
                string assetPath = AssetDatabase.GetAssetPath(assetToDelete);
                Debug.Log($"Deleting NavMesh asset at {assetPath}");
