        ...
        List<NavMeshBuildSource> CollectSources()
        {
            ...
            if (!EditorApplication.isPlaying)
            {
                ...
                for (int i = sources.Count - 1; i >= 0; i--)
                {
                    ...
                    if (source.component == null) continue;
                    StaticEditorFlags flags = GameObjectUtility.GetStaticEditorFlags(source.component.gameObject);
                    // if LocomotionOnly, keep it even if it's not flagged static
                    if (!flags.HasFlag(StaticEditorFlags.NavigationStatic))
                    {
                        if (source.component.gameObject.layer == 15)
                        {
                            Debug.Log($"LocomotionOnly GameObject is not set to static, and isn't included in the navmesh! You may want to change this for the object or prefab: {source.component.gameObject.GetPathFromRoot()}", source.component.gameObject);
                        }
                        sources.RemoveAt(i);
                    }
                }
        ...
        void OnValidate()
        {
            ...
            if (settings.agentTypeID != -1)
            {
                ...
                if (!m_OverrideVoxelSize)
                    m_VoxelSize = settings.agentRadius / 4.0f;
